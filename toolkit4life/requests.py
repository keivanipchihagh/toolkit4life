...
class BaseRequests():
    ...
    def post(self, url: str, data: dict = {}, params: dict = {}, headers: dict = None, threaded: bool = False) -> Response:
        """ POST request """
        thread = Thread(target = self.session.post, kwargs = {"url": url, "data": data, "params": params, "headers": headers, "timeout": self.timeout})
        thread.start()
        if not threaded: return thread.join()


    def delete(self, url: str, data: dict = {}, params: dict = {}, headers: dict = None, threaded: bool = False) -> Response:
        """ DELETE request """
        thread = Thread(target = self.session.delete, kwargs = {"url": url, "data": data, "params": params, "headers": headers, "timeout": self.timeout})
        thread.start()
        if not threaded: return thread.join()


    def put(self, url: str, data: dict = {}, params: dict = {}, headers: dict = None, threaded: bool = False) -> Response:
        """ PUT request """
        thread = Thread(target = self.session.put, kwargs = {"url": url, "data": data, "params": params, "headers": headers, "timeout": self.timeout})
        thread.start()
        if not threaded: return thread.join()